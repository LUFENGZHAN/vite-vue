...
const options = {
    ...
    messageField: ['errorMsg', 'msg', 'message','error'],
    codeField: 'code',
    successCode: [0, 200],
    key: 'c6f8a0d1b7c247c9e6d92b349e1c8b58c47a1ab5f6a36fffbf4b39b7459ce61a',
}
const axiosCreate = axios.create({ baseURL: import.meta.env.VITE_API, timeout: 15000, withCredentials: true })
...
axiosCreate.interceptors.response.use(
    ...
    error => {
       ...
       const messageKey = options.messageField.find(key => data[key]);
        
        const message = messageKey ? data[messageKey] : options.defaultMessage;
        if (data[options.codeField] === 401) {
            const session = requestConfig.sessionStorage ? sessionStorage : localStorage
            session.clear()
